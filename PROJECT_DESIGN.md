...
- **Runtime correction role isolation**: Framework/runtime retry prompts such as tool-only violations, malformed plain-text JSON, empty assistant responses, and empty `send_message` corrections are internal system feedback, not user messages. AgenticBlocks is responsible for injecting these correction prompts with the `system` role so local models do not apologize to the user or treat the diagnostic as user-authored content.
