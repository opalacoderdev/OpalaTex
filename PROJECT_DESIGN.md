...
- **Surgical edit tools on the orchestrator**: The chat-orchestrator directly exposes `read_content_pos`, `replace_content_range`, and `write_content_pos` for precise text inspection and line-based edits. This avoids spawning a tool-fragile worker for simple one-line changes and lets the orchestrator verify worker edits before reporting success.
...
- **Thinking default — asymmetric by agent role**: `think=True` is the default **only** for the `memgpt` (chat-orchestrator) and `orchestrator` agents. All worker sub-agents (`worker`, `landscape_planner`, `refinement_agent`) default to `think=False`. The universal fallback in `get_agent_llm_kwargs()` is also `False`.
