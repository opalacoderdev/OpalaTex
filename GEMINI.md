...
1.2. **Do not remove or replace intended functionality to make an error disappear.** A bug report authorizes investigating and repairing the existing behavior, not choosing a different behavior for the user. Before changing platform integrations, investigate executable discovery, environment variables, packaging, permissions, and confinement as applicable. An executable-not-found error does not establish that an integration is unsupported or impossible. For example, replacing Windows Explorer (`explorer.exe`) on WSL2 with a Linux file manager is a behavior change, not a demonstrated fix; do not make that substitution or require the user to install a different application without explicit authorization. Preserve the intended integration while investigating, and report any verification limits clearly. Mocked tests that confirm a replacement command is invoked do not demonstrate that the original problem is solved in the affected environment.
