...
_STRINGS = {
    "en": {
        ...
        "err_system_message_order": "The Ollama chat template for {model} rejected the request because it requires a single system message at the very beginning of the conversation. If the model's documentation confirms this requirement, enable \"Requires a single system message\" for it in Edit Models, then retry.",
        "err_system_message_order_unresolved": "The Ollama chat template for {model} rejected the request over system-message ordering even though \"Requires a single system message\" is already enabled for it. This may be a bug in how the conversation is assembled — please report it with the steps that triggered this turn.",
    ...
    "pt": {
        ...
        "err_system_message_order": "O template de chat do Ollama para o modelo {model} rejeitou a requisição porque exige uma única mensagem de sistema logo no início da conversa. Se a documentação desse modelo confirmar essa exigência, ative \"Requer uma única mensagem de sistema\" para ele em Editar Modelos e tente novamente.",
        "err_system_message_order_unresolved": "O template de chat do Ollama para o modelo {model} rejeitou a requisição por ordenação de mensagem de sistema, mesmo com \"Requer uma única mensagem de sistema\" já ativado para ele. Isso pode ser um bug na montagem da conversa — por favor, reporte com os passos que causaram este turno.",
