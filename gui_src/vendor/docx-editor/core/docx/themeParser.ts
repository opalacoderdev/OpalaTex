 ...
 *
 * Mutates the theme in place. This is safe for round-tripping because
 * `word/theme/theme1.xml` is preserved verbatim on save (see `rezip.ts`); the
 * parsed `Theme` is only consumed for layout/rendering.
 */
