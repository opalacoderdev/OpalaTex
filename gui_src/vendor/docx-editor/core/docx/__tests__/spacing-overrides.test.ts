...
describe('paragraphParser spacingOverrides', () => {
  test('inline w:before is flagged', () => {
    ...
    expect(p.formatting?.spacingOverrides).toEqual({ before: true });
  });
  ...
  test('inline w:after only is flagged', () => {
    ...
    expect(p.formatting?.spacingOverrides).toEqual({ after: true });
  });
  ...
  test('inline both attrs', () => {
    ...
    expect(p.formatting?.spacingOverrides).toEqual({ before: true, after: true });
  });
  ...
  test('paragraph without inline w:spacing has no explicit flags', () => {
    ...
    expect(p.formatting?.spacingOverrides).toBeUndefined();
  });
});
