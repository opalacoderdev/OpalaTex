...
/** The first text run's docFrom inside the table — enough for findTableAt's walk-up. */
...
describe('commits', () => {
  test('commitColumnResize updates columnWidths and the two cell widths', () => {
    const view = makeView();
    commitColumnResize(view, { docFrom: INSIDE_TABLE, colIdx: 0, newLeft: 2000, newRight: 4000 });
  ...
  test('commitRowResize sets the target row height + heightRule', () => {
    const view = makeView();
    commitRowResize(view, { docFrom: INSIDE_TABLE, rowIdx: 0, newHeight: 555 });
  ...
  test('commitRightEdgeResize grows only the targeted column', () => {
    const view = makeView();
    commitRightEdgeResize(view, { docFrom: INSIDE_TABLE, colIdx: 1, newWidth: 5000 });
  ...
  test('commit on a non-table position is a no-op', () => {
    ...
    commitColumnResize(view, { docFrom: 99999, colIdx: 0, newLeft: 1, newRight: 1 });
