import type { WrapTextDirection } from '../../pagination-model/types';
import { constrainWrapMargins } from './paragraphLayout';
import { MIN_WRAP_SEGMENT_WIDTH } from './wrapThresholds';

// Re-exported so `metrics/index.ts` and the float tests keep their import path.
...
export function rectsToFloatingZones(
  ...
  return rects.map((rect) => {
    ...
    const clamped = constrainWrapMargins(leftMargin, rightMargin, contentWidth);
    return {
