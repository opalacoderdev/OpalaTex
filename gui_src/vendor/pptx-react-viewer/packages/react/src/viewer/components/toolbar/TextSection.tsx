...
import { gB, gL, grp, FMT, ATXT, VTXT, pill, ic } from './toolbar-constants';
...
export function TextSection(p: TextSectionProps): React.ReactElement {
	...
	return (
		<>
			...
			{/* ── Paragraph group ── */}
			<RibbonGroup label={t('pptx.ribbon.group.paragraph')}>
			{/* List style */}
				<div className={grp}>
				<button
					type='button'
				...
				{/* Vertical anchor — PowerPoint's "Align Text". Paragraph alignment
				    (above) only moves text horizontally, so this is the control
				    that centres a caption inside a shape. */}
				<div className={grp}>
				{VTXT.map((b, i, a) => {
					const active = (effectiveTs?.vAlign ?? 'top') === b.id;
					return (
						<button
							key={b.id}
							type='button'
							...
							onClick={() => {
								...
								p.onUpdateTextStyle({ vAlign: b.id });
							}}
							className={cn(
								i < a.length - 1 ? gB : gL,
