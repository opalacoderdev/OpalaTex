...
import type { SparklineData } from './chart-sparkline';

// ── Helper ──────────────────────────────────────────────────────────────

/** Parse an SVG string and return basic info about its structure. */
function parseSvg(svg: string) {
	const widthMatch = svg.match(/width="(?<v>\d+)"/u);
	const heightMatch = svg.match(/height="(?<v>\d+)"/u);
	const polylineMatch = svg.match(/<polyline[^/]*\/>/u);
	const rects = [...svg.matchAll(/<rect[^/]*\/>/gu)];
	const fillMatches = [...svg.matchAll(/fill="(?<v>[^"]*)"/gu)];
	const strokeMatch = svg.match(/stroke="(?<v>[^"]*)"/u);
	const pointsMatch = svg.match(/points="(?<v>[^"]*)"/u);
	return {
		width: widthMatch ? Number(widthMatch.groups?.v) : undefined,
		height: heightMatch ? Number(heightMatch.groups?.v) : undefined,
		hasPolyline: Boolean(polylineMatch),
		rectCount: rects.length,
		fills: fillMatches.map((m) => m.groups?.v),
		stroke: strokeMatch ? strokeMatch.groups?.v : undefined,
		points: pointsMatch ? pointsMatch.groups?.v : undefined,
		raw: svg,
	};
}
...
describe('renderSparklineSvg — winLoss type', () => {
	...
	it('should have fixed-height bars (all same height)', () => {
		...
		const svg = renderSparklineSvg(data);
		const heightMatches = [...svg.matchAll(/height="(?<v>[^"]*)"/gu)];
		// First match is the SVG element itself; remaining are the rect elements
		const rectHeights = heightMatches.slice(1).map((m) => parseFloat(m.groups?.v ?? ''));
	...
	it('should position positive bars in the top half', () => {
		...
		const svg = renderSparklineSvg(data);
		const yMatch = svg.match(/<rect[^>]*y="(?<v>[^"]*)"/u);
		expect(yMatch).toBeTruthy();
		const y = parseFloat(yMatch!.groups?.v ?? '');
